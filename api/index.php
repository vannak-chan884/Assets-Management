...
function handleDevices($method, $id, $pdo)
{
    if ($method === 'GET') {
        ...
        $d = getJsonInput();
        // Allow updating property number (device_id)
        $stmt = $pdo->prepare("
            UPDATE device 
            SET device_id = ?, device_name = ?, description = ?
            WHERE device_id = ?
        ");
        $stmt->execute([
            $d['device_id'],       // new property number
            $d['device_name'],
            $d['description'],
            $id                    // old property number
        ]);
        jsonResponse(['ok' => true]);
    } elseif ($method === 'DELETE' && $id) {
...
function handleAssignments($method, $id, $pdo)
{
    if ($method === 'GET') {
        ...
        jsonResponse(['error' => 'Not allowed'], 405);
}
...
function handleAccounts($method, $id, $pdo)
{
    ...
    if (!$sub)
        jsonResponse(['error' => 'sub param required, e.g. ?sub=domain'], 400);
    ...
    if (!isset($tableMap[$sub]))
        jsonResponse(['error' => 'invalid sub'], 400);
    $table = $tableMap[$sub];
